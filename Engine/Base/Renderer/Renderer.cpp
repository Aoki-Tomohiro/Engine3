...
void Renderer::PreDrawModels(ModelRenderingType type) {
	...
	//PipelineStateを設定
	commandList_->SetPipelineState(modelPipelineStates_[type].Get());
}
...
void Renderer::PreDrawSprites(BlendMode blendMode) {
	...
	//PipelineStateを設定
	commandList_->SetPipelineState(spritePipelineStates_[blendMode].Get());
}
...
void Renderer::PreDrawParticles() {
	...
	//PipelineStateを設定
	commandList_->SetPipelineState(particlePipelineStates_[0].Get());
}
...
void Renderer::CreateModelPipelineState() {
	...
	//実際に生成
	hr = device_->CreateGraphicsPipelineState(&graphicsPipelineStateDesc, IID_PPV_ARGS(&pipelineState[Opaque]));
	assert(SUCCEEDED(hr));
	//配列に追加
	...
	//生成
	hr = device_->CreateGraphicsPipelineState(&graphicsPipelineStateDesc, IID_PPV_ARGS(&pipelineState[Transparent]));
	assert(SUCCEEDED(hr));
	//配列に追加
...
void Renderer::CreateSpritePipelineState() {
	...
	graphicsPipelineStateDesc.DSVFormat = DXGI_FORMAT_D24_UNORM_S8_UINT;
	//実際に生成
	hr = device_->CreateGraphicsPipelineState(&graphicsPipelineStateDesc, IID_PPV_ARGS(&pipelineState[kBlendModeNone]));
	assert(SUCCEEDED(hr));
	//配列に追加
	...
	graphicsPipelineStateDesc.BlendState = blendDesc;
	//実際に生成
	hr = device_->CreateGraphicsPipelineState(&graphicsPipelineStateDesc, IID_PPV_ARGS(&pipelineState[kBlendModeNormal]));
	assert(SUCCEEDED(hr));
	//配列に追加
	...
	graphicsPipelineStateDesc.BlendState = blendDesc;
	//実際に生成
	hr = device_->CreateGraphicsPipelineState(&graphicsPipelineStateDesc, IID_PPV_ARGS(&pipelineState[kBlendModeAdd]));
	assert(SUCCEEDED(hr));
	//配列に追加
	...
	graphicsPipelineStateDesc.BlendState = blendDesc;
	//実際に生成
	hr = device_->CreateGraphicsPipelineState(&graphicsPipelineStateDesc, IID_PPV_ARGS(&pipelineState[kBlendModeSubtract]));
	assert(SUCCEEDED(hr));
	//配列に追加
	...
	graphicsPipelineStateDesc.BlendState = blendDesc;
	//実際に生成
	hr = device_->CreateGraphicsPipelineState(&graphicsPipelineStateDesc, IID_PPV_ARGS(&pipelineState[kBlendModeMultiply]));
	assert(SUCCEEDED(hr));
	//配列に追加
	...
	graphicsPipelineStateDesc.BlendState = blendDesc;
	//実際に生成
	hr = device_->CreateGraphicsPipelineState(&graphicsPipelineStateDesc, IID_PPV_ARGS(&pipelineState[kBlendModeScreen]));
	assert(SUCCEEDED(hr));
	//配列に追加
...
void Renderer::CreateParticlePipelineState() {
	...
	graphicsPipelineStateDesc.DSVFormat = DXGI_FORMAT_D24_UNORM_S8_UINT;
	//実際に生成
	hr = device_->CreateGraphicsPipelineState(&graphicsPipelineStateDesc, IID_PPV_ARGS(&pipelineState));
	assert(SUCCEEDED(hr));
	//配列に追加
